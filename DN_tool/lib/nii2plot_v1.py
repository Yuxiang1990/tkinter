...
def parse_args():
    ...
    parser.add_argument('-nii_mask', '--nii_mask',
                        help='nii input, mask')
...
def nrrd2nii(nii_mask_path, nii_path):
    # arr
    ...
    nii_mask = nii_mask.get_data()
    mask = np.transpose(nii_mask, (2, 1, 0))
...
def nii_draw(img_path, mask_path, out):
...
if __name__ == "__main__":
    args = parse_args()
    mask, raw, spacezyx = nrrd2nii(args.nii_mask, args.nii)
    crop_raw_pad, crop_mask_pad, pad, cropsize = _crop(raw, mask)
    title = os.path.basename(args.nii_mask)
